<?php

namespace App\Http\Middleware;

use Closure;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Symfony\Component\HttpFoundation\Response;

class CommitteeMiddleware
{
    public function handle(Request $request, Closure $next): Response
    {
        if (!Auth::check()) {
            return redirect()->route('login')->with('error', 'Please login to access this section.');
        }

        $user = Auth::user();
        $activeRole = $request->session()->get('active_role', $user->role);

        // active_role can only be 'Committee' via AuthController::login()'s validated
        // grantedRoles() check (primary role or an explicit RoleGrantService grant), so
        // checking it alone is sufficient — no need to also require users.role === 'Committee'
        // (that's exactly what blocked a legitimately granted, non-primary Committee login).
        if ($activeRole === 'Committee') {
            return $next($request);
        }

        $redirectRoute = match ($activeRole) {
            'Admin' => 'admin.dashboard',
            'Devotee' => 'devotee.dashboard',
            'Priest' => 'priest.dashboard',
            'Trustee' => 'trustee.dashboard',
            'Staff' => 'staff.dashboard',
            'Accountant' => 'accountant.dashboard',
            'Committee' => 'committee.dashboard',
            default => 'login',
        };

        return redirect()->route($redirectRoute)->with('error', 'Unauthorized access.');
    }
}
