...
<div class="container-fluid px-4 py-4">
    ...
    <div class="page-header animate__animated animate__fadeIn">
        <div>
            <h1><i class="bi bi-people-fill"></i> Manage Devotees</h1>
            <div class="subtitle">
                <i class="bi bi-database me-1"></i> Total Devotees: <strong>{{ $devotees->count() }}</strong>
    ...
    <div class="table-card animate__animated animate__fadeInUp">
        <div class="card-header">
            <span><i class="bi bi-table me-2" style="color:#b8863a;"></i> Devotee List</span>
            <span class="table-info">
                <i class="bi bi-search me-1"></i> Search & manage temple devotees
            </span>
        </div>
        <div style="padding: 0 20px 20px 20px; overflow-x: auto;">
            <table class="table table-striped" id="devoteesTable">
                <thead>
                    <tr>
                        <th>ID</th>
                        <th>Name</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td><strong>DEV{{ str_pad($devotee->devotee_id, 4, '0', STR_PAD_LEFT) }}</strong></td>
                        <td><i class="bi bi-person-circle me-2" style="color:#b8863a;"></i> {{ $devotee->name }}</td>
                        <td>{{ $devotee->mobile }}</td>
                        <td>{{ $devotee->email }}</td>
                        <td>{{ $devotee->gothra ?? 'N/A' }}</td>
                        <td>{{ $devotee->nakshatra ?? 'N/A' }}</td>
                        <td>
                            @if($devotee->membership_name)
                                <span class="badge bg-success bg-opacity-10 text-success fw-bold">
                                    {{ $devotee->membership_name }}
                                </span>
                            @else
                        ...
                        <td>
                            @if($devotee->verified)
                                <span class="badge-verified yes"><i class="bi bi-check-circle-fill"></i> Yes</span>
                            @else
                                <span class="badge-verified no"><i class="bi bi-x-circle-fill"></i> No</span>
                            @endif
                        </td>
                        <td>{{ date('Y-m-d', strtotime($devotee->created_at)) }}</td>
                        <td>
                            <button class="btn-action view viewBtn"
                                data-bs-toggle="modal"
                                data-bs-target="#viewDevoteeModal"
                                data-id="{{ $devotee->devotee_id }}"
                                data-name="{{ $devotee->name }}"
                                data-mobile="{{ $devotee->mobile }}"
                                data-email="{{ $devotee->email }}"
                                data-gender="{{ $devotee->gender ?? 'N/A' }}"
                                data-dob="{{ $devotee->dob ?? 'N/A' }}"
                                data-gothra="{{ $devotee->gothra ?? 'N/A' }}"
                                data-nakshatra="{{ $devotee->nakshatra ?? 'N/A' }}"
                                data-membership_id="{{ $devotee->membership_id ?? '' }}"
                                data-membership_name="{{ $devotee->membership_name ?? 'None' }}"
                                data-membership_start_date="{{ $devotee->membership_start_date ?? 'N/A' }}"
                                data-membership_end_date="{{ $devotee->membership_end_date ?? 'N/A' }}"
                                data-verified="{{ $devotee->verified }}"
                                data-address="{{ $devotee->address ?? 'N/A' }}">
                            ...
                            <button class="btn-action edit editBtn"
                                ...
                                data-mobile="{{ $devotee->mobile }}"
                                data-email="{{ $devotee->email }}"
                                data-gender="{{ $devotee->gender ?? '' }}"
                                data-dob="{{ $devotee->dob ?? '' }}"
                                data-gothra="{{ $devotee->gothra ?? '' }}"
                                data-nakshatra="{{ $devotee->nakshatra ?? '' }}"
                                data-membership_id="{{ $devotee->membership_id ?? '' }}"
                                data-verified="{{ $devotee->verified }}"
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header border-0 pb-0">
                <h5 class="modal-title fw-bold"><i class="bi bi-exclamation-triangle-fill me-2 text-danger"></i>Delete Devotee</h5>
                <button type="button" class="close-btn" data-bs-dismiss="modal">
                    <i class="bi bi-x-lg"></i>
                </button>
            </div>
            <div class="modal-body pt-3">
                <p class="mb-2" style="font-weight: 450; color: #2d1f0e;">Are you sure you want to delete this devotee?</p>
                <p class="text-muted" style="font-size:0.95rem;">
                    <strong>Devotee:</strong> <span id="deleteDevoteeName"></span><br>
                    <strong>ID:</strong> DEV<span id="deleteDevoteeId"></span>
                </p>
...
<!-- VIEW DEVOTEE MODAL -->
<div class="modal fade" id="viewDevoteeModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-person-badge"></i> Devotee Details</h5>
                <button type="button" class="close-btn" data-bs-dismiss="modal">
                    <i class="bi bi-x-lg"></i>
                </button>
            </div>
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        <label>Gothra</label>
                        <input type="text" id="view_gothra" class="form-control" readonly>
                    </div>
                    <div class="col-md-6 mb-3">
                        <label>Nakshatra</label>
                        <input type="text" id="view_nakshatra" class="form-control" readonly>
                    </div>
                    <div class="col-12 mb-3">
                        <label>Address</label>
                    ...
                    <div class="col-12 mb-3 mt-2">
                        <h6 class="text-muted" style="font-weight:700; border-bottom:2px solid #b8863a; padding-bottom:8px;">
                            <i class="bi bi-shield-check me-2" style="color:#b8863a;"></i>Membership & Verification
                        </h6>
                    </div>
                    <div class="col-md-6 mb-3">
                        <label>Membership Tier</label>
                        <input type="text" id="view_membership_name" class="form-control" readonly>
                    </div>
                    <div class="col-md-6 mb-3">
                        <label>Verified Status</label>
                        <input type="text" id="view_verified" class="form-control" readonly>
                    </div>
                    <div class="col-md-6 mb-3">
                        <label>Membership Start Date</label>
                        <input type="text" id="view_membership_start" class="form-control" readonly>
                    </div>
                    <div class="col-md-6 mb-3">
                        <label>Membership End Date</label>
                        <input type="text" id="view_membership_end" class="form-control" readonly>
                    </div>
                </div>
            </div>
...
<!-- EDIT DEVOTEE MODAL -->
<div class="modal fade" id="editDevoteeModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-pencil-square"></i> Edit Devotee</h5>
                <button type="button" class="close-btn" data-bs-dismiss="modal">
                    <i class="bi bi-x-lg"></i>
                </button>
            </div>
            <div class="modal-body">
                <form id="editForm" method="POST" action="">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            <label>Gothra</label>
                            <input type="text" name="gothra" id="edit_gothra" class="form-control">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label>Nakshatra</label>
                            <input type="text" name="nakshatra" id="edit_nakshatra" class="form-control">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label>Membership</label>
                            <select name="membership_id" id="edit_membership_id" class="form-select">
                        ...
                        <div class="col-md-6 mb-3">
                            <label>Verified Status</label>
                            <select name="verified" id="edit_verified" class="form-select" required>
                    ...
                    <button type="submit" class="btn btn-success w-100">
                        <i class="bi bi-check-circle me-2"></i> Update Devotee
                    </button>
                </form>
            </div>
        </div>
    ...
    $(document).ready(function() {
        ...
        $('#devoteesTable').DataTable({
            ...
            language: {
                ...
                info: "Showing _START_ to _END_ of _TOTAL_ devotees",
                infoEmpty: "No devotees found",
                infoFiltered: "(filtered from _MAX_ total devotees)"
            },
            order: [[0, 'asc']],
            columnDefs: [
        ...
        // Delete Modal - Set devotee details
        $('#deleteModal').on('show.bs.modal', function(event) {
            const button = $(event.relatedTarget);
            const devoteeId = button.data('devotee-id');
            const devoteeName = button.data('devotee-name');
            const modal = $(this);
            modal.find('#deleteDevoteeId').text(devoteeId);
            modal.find('#deleteDevoteeName').text(devoteeName);
            modal.find('#deleteForm').attr('action', '/admin/devotee/delete/' + devoteeId);
        });
        ...
        $('.viewBtn').on('click', function() {
            ...
            $('#view_id').val('DEV' + String(data.id).padStart(4, '0'));
            ...
            $('#view_gothra').val(data.gothra);
            $('#view_nakshatra').val(data.nakshatra);
            $('#view_membership_name').val(data.membership_name);
            $('#view_verified').val(data.verified == 1 ? 'Verified' : 'Not Verified');
            $('#view_membership_start').val(data.membership_start_date);
            $('#view_membership_end').val(data.membership_end_date);
            $('#view_address').val(data.address);
        });

        // Edit Modal - Populate all fields
        $('#editDevoteeModal').on('show.bs.modal', function(event) {
            const button = $(event.relatedTarget);
            const data = button.data();
            
            $('#editForm').attr('action', '/admin/devotee/update/' + data.id);
            ...
            $('#edit_gothra').val(data.gothra);
            $('#edit_nakshatra').val(data.nakshatra);
            $('#edit_membership_id').val(data.membership_id);
            $('#edit_verified').val(data.verified);
